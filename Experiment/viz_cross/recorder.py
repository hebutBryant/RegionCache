...
if __name__ == "__main__":
    ...
    prompt="a cat play with a ball"
    # 正常推理即可
    result = pipe(prompt=prompt, num_inference_steps=20, guidance_scale=4.0)

    # 2) 保存图片
    img = result.images[0]   # PIL.Image
    img_path = "generate.png"
    img.save(img_path)
    ...
    print("🧩 Tokens (index : id -> text):")
    for i in range(prompt_len):
        token_id = int(input_ids[i].item())

        # 将单个 token id 解码为字符串
        token_text = tok.decode(
            [token_id],
            skip_special_tokens=False,
            clean_up_tokenization_spaces=False
        )

        print(f"  [{i:02d}] {token_id:6d} -> {repr(token_text)}")
        

    # === 2) 只保存最后一步的 cross-attn，并在最后一维裁到 prompt_len ===
    save_json = "cross_attn_laststep.json"
    dump = {}
